...
import { fileDataSchema, messageDataSchema } from "../../validation/socket/chatSocket.validation";
import { SocketEventsEnum } from "../../models/enums/socket.enum";
import { ChatRepository } from "../../repository/chat.repository";
import { ErrorEnum } from "../../models/enums/error.enum";
import { writeFile } from "fs";
import { v4 as uuidv4 } from 'uuid';
import { JwtService, redisClient } from "../../utils";
...
export class ChatSocket implements ISocket {
    ...
    handleConnection(socket: Socket) {
        ...
        socket.on(SocketEventsEnum.MSG_SEND, async (data) => {
            try {
                ...
                socket.emit(SocketEventsEnum.ERROR, error);
                console.error(error);
            }
        });
    ...
    async middlewareImplementation(socket: Socket, next: (arg0?: Error) => void) {
        try {
            ...
            const userData = await JwtService.verify(socket.handshake.headers.authorization);
